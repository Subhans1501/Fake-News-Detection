...
if isinstance(_models_or_error,Exception):
    st.error("Model loading failed. Please check your model paths in ensemble.py.")
        ...
        st.warning("Please enter news text before analyzing.")
            ...
            if mode.startswith("Ensemble"):
                try:
                    ...
                    st.error("Error during ensemble prediction.")
                    st.exception(e)
                    st.stop()
                ...
                for col,(mname,info) in zip(cols, result["per_model"].items()):
                    pred_label="Real" if info["pred"]==1 else "Fake"
                    col.markdown(f"**{mname.upper()}**")
                    col.markdown(f"- Prediction: **{pred_label}**")
                    col.markdown(f"- Probabilities: `{info['probs']}`")
                epred=result["ensemble"]["pred"]
                econf=result["ensemble"]["confidence"]
                method=result["ensemble"]["method"]
                label_txt="🟢 REAL NEWS" if epred == 1 else "🔴 FAKE NEWS"
                st.markdown("---")
                st.markdown(f"### 🏁 Final Ensemble Decision ({method})")
                st.markdown(
                    f"<div style='padding:15px; font-size:22px; font-weight:bold; "
                    f"border-radius:10px; background:#1f77b4; text-align:center;'>"
                    f"{label_txt}<br><span style='font-size:16px;'>Confidence: {econf:.2f}</span>"
                    f"</div>",
                    unsafe_allow_html=True
                )
            else:
                selected=st.selectbox("Select Model",list(models.keys()))
                with st.spinner(f"Predicting with {selected.upper()}..."):
                    info=predict_with_model(text,selected,models)
                pred_label="Real" if info["pred"]==1 else "Fake"
                st.markdown("### 🔍 Single Model Result")
                st.markdown(f"- Model: **{selected.upper()}**")
                st.markdown(f"- Prediction: **{pred_label}**")
                st.markdown(f"- Probabilities: `{info['probs']}`")
st.markdown("---")
st.markdown(
    """
